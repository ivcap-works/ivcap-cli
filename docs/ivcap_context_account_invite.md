...
Invite a user (by email) to an account, granting the given capabilities when they
...
```
ivcap context account invite account_id --email <email> [--capability <cap> ...] [flags]
```

### Options

```
  -c, --capability strings   Capability to grant on accept (repeatable). Run 'ivcap context capabilities --kind account' to list valid values
