## ivcap nextflow list-jobs

List recent Nextflow jobs

### Synopsis

List jobs that were created with Nextflow services (jobs with nextflow request schema)

```
ivcap nextflow list-jobs [flags]
```
...
```
  ...
  -c, --content-path string   json path filter on job's content ('$.images[*] ? (@.size > 10000)')
