...
func filterToolsBySessionAllowlist(ctx context.Context, tools []mcp.Tool) []mcp.Tool {
	// If there is no allowlist for the current session, behave as if only built-ins are allowed.
...
func getDiscoveryState(s *server.MCPServer) *mcpDiscoveryState {
	...
	// Use a package-level map keyed by pointer. This is safe because
...
func discoverToolsViaServiceSearch(ctx context.Context, interest string, s *server.MCPServer) (map[string]mcpDiscoveredTool, map[string]float64, []string, []string, error) {
	...
	for _, serviceID := range serviceIDs {
		...
		aspects, _, err := listAspectFn(ctx, selector, adpt, srvCfg.Logger)
		if err != nil {
			if isAuthFailure(err) {
				return nil, nil, nil, nil, ErrLoginRequired
			}
...
func runTool(ctx context.Context, serviceID string, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	if srvCfg.Logger != nil {
		srvCfg.Logger.Info("Calling service", log.String("service-id", serviceID), log.Reflect("params", request.Params))
	}
