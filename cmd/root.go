...
func CreateDoc() {
	...
	// Normalize timestamp to avoid spurious git changes.
	...
	// Open a root-scoped handle to prevent path traversal and symlink TOCTOU
	// (gosec G304, G703, G122). All file operations inside the walk use this
	// root rather than the raw `path` string from the callback.
	const docDir = "./doc"
	docRoot, rootErr := os.OpenRoot(docDir)
	if rootErr != nil {
		logger.Fatal("while opening doc root", log.Error(rootErr))
	}
	defer func() {
		if err := docRoot.Close(); err != nil {
			logger.Error("while closing doc root", log.Error(err))
		}
	}()
	...
	err = filepath.WalkDir(docDir, func(path string, d fs.DirEntry, err error) error {
		...
		relPath, relErr := filepath.Rel(docDir, path)
		if relErr != nil {
		...
		updated := timestampPattern.ReplaceAll(content,
	...
	if err != nil {
		logger.Fatal("while normalizing doc timestamps", log.Error(err))
	}
}
